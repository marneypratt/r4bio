...
This tutorial assumes you understand how to use the pipe operator `%>%` and some other basic R skills. See the other **Intro to R for BIology** tutorials for review.

Like many of the tutorials in **Intro to R for Biology**, we will use the hemlocks dataset from the `r4bio` package. 

As a reminder, the hemlocks data were collected by Smith College Bio 131 students. They measured the diameter and height for eastern hemlock trees at 4 locations in Western Massachusetts. You can get more information by searching for `hemlocks` in the help tab in RStudio or typing `?hemlocks` in the RStudio Console.
...
See this link [Histograms](https://moderndive.com/2-viz.html#histograms) to learn more about historgrams.
...
The general form for the `summarize()` function using the pipe operator is
