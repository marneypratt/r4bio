...
Let's say you want to take the original hemlocks dataset and filter for trees from the Smith College ("SMC") and then select just the date, diameter, and height variables.  You could use the following code to do this using the native R pipe operator.

```{r pipes, out.width = "70%"}
knitr::include_graphics("images/nativePipe.png")
```
...
Use the native R pipe operator to link together two functions from `{dplyr}` so that the `hemlocks4` data frame gives data for just the "Sapling" trees and arrange by diameter in descending order.  Use the skeleton of the code given below and just fill in the blanks.
